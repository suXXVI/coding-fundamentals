...
window.addEventListener('load', async function() {
    ...
    if (code) {
        ...
        await fetchUserId(accessToken);
        await getUserPlaylists(accessToken);
        await fetchDiscoverWeekly(accessToken);
    } else {
        requestAuthorization();
    };
});
...
////// authentication with pkce, following spotify guide //////
...
// gets user_id
async function fetchUserId(endpoint, method, body) {
...
//     try {
//         let endpoint = `https://api.spotify.com/`;
...
// getting playlist_id for Discover Weekly playlist
async function getUserPlaylists(accessToken) {
    try {
        let endpoint = `https://api.spotify.com/v1/me/playlists`;
        let response = await fetch(endpoint + '?limit=20', {
            method: 'GET',
            headers: {
                Authorization: `Bearer ${accessToken}`,
            },
        ...
        let discoverWeeklyItem = playlists.find(item => item.name === 'Discover Weekly');
        let playlistId = discoverWeeklyItem.id;

        localStorage.setItem('playlist_id', playlistId); // save playlist_id locally
    } catch (error) {
        console.log(error);
    };  
};
...
// using the access token to get tracks from playlist
async function fetchDiscoverWeekly(accessToken) {
    ...
    try {
        ...
        let response = await fetch(endpoint + '?limit=30', {
            method: 'GET',
            headers: {
                Authorization: 'Bearer ' + accessToken,
            },
...
// using the access token to get and display tracks from New Playlist //
async function fetchNewPlaylist(accessToken) {
    let playlist_id = localStorage.getItem('playlist_id')
    try {
        let endpoint = `https://api.spotify.com/v1/playlists/${playlist_id}/tracks`;
        let response = await fetch(endpoint + '?limit=30', {
...
async function createPlaylist(tracksUris) {
    const { id: user_id } = await fetchUserId('v1/me', 'GET')
    const tracksUri = tracksUris

    const playlist = await fetchUserId(
